...
def plot_gmm_3d(
    ...
    if pts is not None and len(pts) > 0:
        ...
        dp = (
            pts
            if len(pts) <= max_pts
            else pts[np.random.choice(len(pts), max_pts, replace=False)]
        )
...
def save_gmm_to_file(gmm, filepath: str):
    ...
    print(
        f"[save_gmm] Saved {gmm.n_components_} components → {os.path.basename(filepath)}"
    )


def plot_two_gmms_3d(
    gmm_a,
    gmm_b,
    ...
    def _draw_gmm(ax, gmm, color, label, max_els):
        ...
        ax.scatter(
            *gmm.means_.T, c=color, s=50, zorder=5, label=label, depthshade=False
        )
    ...
    for pts, color, name in [
        (pts_a, "salmon", "Cloud A"),
        (pts_b, "royalblue", "Cloud B"),
    ]:
        if pts is not None and len(pts) > 0:
            max_pts = 10_000
            dp = (
            ...
            ax.scatter(
                dp[:, 0], dp[:, 1], dp[:, 2], c=color, s=1, alpha=0.25, label=name
            )
...
class TestSimpleGMMReg:
    def __init__(self):
        ...
        self.sogmm_compute = rospy.get_param("~sogmm_compute", "GPU")  # "CPU" or "GPU"

        if not HAS_SOGMM:
            raise ImportError(
                "sogmm_py not found - cannot run tests. "
                f"Expected venv at {_RECONSTRUCTION_VENV}"
            )
        else:
        ...
        if not HAS_GMM_REGISTRATION:
            raise ImportError(
                "gmm_d2d_registration_py not found - cannot run tests. "
                f"Expected venv at {_REGISTRATION_VENV}"
            )
